...
const RequestSchema = z.object({
  ...
  persona: z.enum(["jobs", "mckinsey"]).optional(),
...
// ✅ [수정 1] extractJSON 강화 - 배열([])도 추출하고, 중첩 파싱 실패 시 재시도
function extractJSON(text: string): any | null {
  if (!text || typeof text !== "string") return null;

  // 1) ```json ... ``` 또는 ``` ... ``` 코드블록 우선 처리
...
// ✅ [수정 2] JSON 전용 시스템 프롬프트 접미사 - 모든 프롬프트에 추가
const JSON_ONLY_SUFFIX = `

[절대 규칙] 반드시 순수한 JSON만 응답하세요.
- 응답의 첫 글자는 반드시 { 또는 [ 이어야 합니다.
- \`\`\`json 코드블록을 절대 사용하지 마세요.
- "네, 알겠습니다" 같은 설명 텍스트를 앞뒤에 절대 붙이지 마세요.
- JSON 내부의 문자열에서 줄바꿈이 필요하면 반드시 \\n 으로 이스케이프하세요.`;
...
serve(async (req) => {
  ...
  try {
    ...
    if (!authHeader?.startsWith("Bearer ")) {
      return new Response(JSON.stringify({ error: "인증이 필요합니다. 로그인해주세요." }), { status: 401, headers: { ...corsHeaders, "Content-Type": "application/json" } });
    }
    ...
    if (userError || !userData?.user) {
      return new Response(JSON.stringify({ error: "인증이 만료되었습니다. 다시 로그인해주세요." }), { status: 401, headers: { ...corsHeaders, "Content-Type": "application/json" } });
    }

    const rawBody = await req.text();
    if (rawBody.length > MAX_BODY_SIZE) throw new Error("요청 데이터가 너무 큽니다.");
    ...
    else if (mode === "change_persona") return await handleChangePersona(body, LOVABLE_API_KEY);
    ...
    return new Response(JSON.stringify({ error: safeMessage }), { status, headers: { ...corsHeaders, "Content-Type": "application/json" } });
  }
});
...
아래 JSON 형식으로만 반환하세요:
{
  "slide": {
    ...
    "chartData": ${currentSlide.chartData ? JSON.stringify(currentSlide.chartData) : 'null'},
    "tableData": ${currentSlide.tableData ? JSON.stringify(currentSlide.tableData) : 'null'}
  }
}${JSON_ONLY_SUFFIX}`;
...
아래 JSON 형식으로 목차만 생성하세요:
{
  ...
  // ✅ [수정 3] outline 파싱 강화 - extractJSON 실패 시 상세 로그 후 명확한 에러
  console.log("[outline] AI raw response (first 500 chars):", content.slice(0, 500));

  const outline = extractJSON(content);
  if (!outline) {
    console.error("[outline] JSON parse failed. Full response:", content);
  ...
  if (!outline.title || !Array.isArray(outline.outline) || outline.outline.length === 0) {
    console.error("[outline] Invalid outline structure:", JSON.stringify(outline));
...
참고: chartData나 tableData는 필요하고 적절한 슬라이드에만 선택적으로 포함하세요.${JSON_ONLY_SUFFIX}`;

  const data = await callAI(prompt, apiKey);
  const content = data.choices?.[0]?.message?.content || "";

  console.log("[generate] AI raw response (first 500 chars):", content.slice(0, 500));

  const presentation = extractJSON(content);
  if (!presentation) {
    console.error("[generate] JSON parse failed. Full response:", content);
    throw new Error("AI가 올바른 JSON 형식으로 응답하지 않았습니다. 다시 시도해주세요.");
  }
  if (!presentation.title || !Array.isArray(presentation.slides) || presentation.slides.length === 0) {
    console.error("[generate] Invalid presentation structure:", JSON.stringify(presentation).slice(0, 300));
    throw new Error("AI가 올바른 슬라이드 구조를 생성하지 못했습니다. 다시 시도해주세요.");
  }

  return new Response(JSON.stringify({ presentation, rawContent: content }), { headers: { ...corsHeaders, "Content-Type": "application/json" } });
}
...
아래 JSON 형식으로 슬라이드 1개만 반환하세요:
{
  ...
  if (!slide) throw new Error("슬라이드 JSON을 파싱할 수 없습니다. 다시 시도해주세요.");
...
요청을 정확히 반영하여 슬라이드를 수정하고, 아래 JSON 형식으로만 반환하세요:
{
  "slide": {
    "slideNumber": ${slideIndex + 1},
    "title": "슬라이드 제목",
  ...
  const content = data.choices?.[0]?.message?.content || "";
  const result = extractJSON(content);
  if (!result) throw new Error("AI 수정 결과를 파싱할 수 없습니다. 다시 시도해주세요.");
...
아래 JSON 형식으로만 반환하세요:
{
  ...
  if (!review) throw new Error("리뷰 결과를 파싱할 수 없습니다. 다시 시도해주세요.");
...
아래 JSON 형식으로 개선된 전체 발표 자료와 수정 요약을 반환하세요:
{
  ...
  const content = data.choices?.[0]?.message?.content || "";
  const result = extractJSON(content);
  if (!result) throw new Error("최적화 결과를 파싱할 수 없습니다. 다시 시도해주세요.");
...
async function handleGenerateImage(body: any, apiKey: string) {
  ...
  const imagePrompt = customPrompt || `Create a professional, clean, minimalist business presentation background image for a slide about: "${slideTitle}". Context: ${(slideContent || []).slice(0, 2).join('. ')}. Slide type: ${slideType}. Style: Corporate, modern, subtle gradient or abstract geometric shapes. No text. Suitable as a background or accent image for a presentation slide. 16:9 aspect ratio. High quality.`;
  ...
  if (!imageData) throw new Error("이미지를 생성하지 못했습니다. 다시 시도해주세요.");
  ...
  const { error: uploadError } = await supabase.storage.from("slide-images").upload(fileName, binaryData, { contentType: "image/png", upsert: true });
...
// ✅ [수정 4] callAI - generationConfig 제거, 올바른 OpenAI 호환 파라미터 사용
  ...
  try {
    const response = await fetch("https://ai.gateway.lovable.dev/v1/chat/completions", {
      ...
      body: JSON.stringify({
        ...
        // ✅ generationConfig 제거 → OpenAI 호환 파라미터로 교체
        temperature: 0.2,
        max_tokens: 8192,
      }),
...
async function handleSearchImages(body: any) {
  ...
  if (!res.ok) throw new Error("이미지 검색 중 오류가 발생했습니다.");

  const data = await res.json();
  const images = data.results.map((img: any) => ({
    id: img.id, url: img.urls.regular, thumbUrl: img.urls.small,
  ...
  const res = await fetch("https://ai.lovable.dev/api/chat", {
    ...
    body: JSON.stringify({
      ...
      messages: [{
        ...
        content: [{
          type: "text",
          text: `다음은 PPT 템플릿입니다. 브랜드 스타일 요소를 분석해주세요.
반드시 아래 JSON 형식으로만 응답하세요:
{
  "primaryColor": "#hex", "accentColor": "#hex", "backgroundColor": "#hex", "textColor": "#hex",
  "fontStyle": "고딕|명조|둥근|모던|클래식", "layoutStyle": "미니멀|기업|크리에이티브|데이터중심", "description": "한 줄 설명"
}`
        }, ...(templateData.startsWith("data:") ? [{ type: "image_url" as const, image_url: { url: templateData } }] : [{ type: "text" as const, text: `템플릿 내용:\n${templateData}` }])],
      }],
    }),
  });

  if (!res.ok) throw new Error("템플릿 분석 중 오류가 발생했습니다.");
  const aiData = await res.json();
  const parsed = extractJSON(aiData.choices?.[0]?.message?.content || "");
