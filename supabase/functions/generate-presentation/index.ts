...
async function handleOutline(body: any, apiKey: string) {
  ...
  if (!outline) throw new Error("AI가 올바른 JSON 형식으로 응답하지 않았습니다. 다시 시도해주세요.");
  if (!outline.title || !Array.isArray(outline.outline) || outline.outline.length === 0) throw new Error("AI가 올바른 구성안 구조를 생성하지 못했습니다. 다시 시도해주세요.");
...
async function handleGenerate(body: any, apiKey: string) {
  ...
  if (!presentation) throw new Error("AI가 올바른 JSON 형식으로 응답하지 않았습니다. 다시 시도해주세요.");
  if (!presentation.title || !Array.isArray(presentation.slides) || presentation.slides.length === 0) throw new Error("AI가 올바른 슬라이드 구조를 생성하지 못했습니다. 다시 시도해주세요.");
...
async function callAI(prompt: string, apiKey: string) {
  ...
  try {
    ...
    if (err instanceof DOMException && err.name === "AbortError") throw new Error("AI 응답 시간이 초과되었습니다. 파일 크기를 줄이거나 다시 시도해주세요.");
    throw err;
  } finally {
...
async function handleSearchImages(body: any) {
  ...
  const images = data.results.map((img: any) => ({
    ...
    photographer: img.user.name, photographerUrl: img.user.links.html, downloadUrl: img.links.download_location,
  }));
  return new Response(JSON.stringify({ images, totalPages: data.total_pages, total: data.total }), { headers: { ...corsHeaders, "Content-Type": "application/json" } });
}
