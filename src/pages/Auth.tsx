...
export default function Auth() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      let friendlyMessage = '오류가 발생했습니다. 다시 시도해 주세요.';
      ...
      if (msg.includes('Invalid login credentials') || msg.includes('invalid_credentials')) {
        ...
        friendlyMessage = '이미 가입된 이메일입니다. 로그인을 이용해 주세요.';
      } else if (msg.includes('Password should be at least')) {
        friendlyMessage = '비밀번호는 6자 이상이어야 합니다.';
      } else if (msg.includes('Unable to validate email address')) {
        friendlyMessage = '올바른 이메일 형식을 입력해 주세요.';
      } else if (msg.includes('network') || msg.includes('fetch')) {
        friendlyMessage = '네트워크 연결을 확인해 주세요.';
      } else if (msg.includes('rate_limit') || msg.includes('too many')) {
        friendlyMessage = '요청이 너무 많습니다. 잠시 후 다시 시도해 주세요.';
      }
