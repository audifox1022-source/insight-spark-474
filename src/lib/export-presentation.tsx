...
export async function exportToPptx(
  ...
  for (const slide of presentation.slides) {
    ...
    if (slide.type === 'title') {
      ...
      // 장식용 패턴
      s.addShape(pptx.ShapeType.oval, { x: -2, y: -2, w: 6, h: 6, fill: { color: WHITE, transparency: 90 } });
      s.addShape(pptx.ShapeType.oval, { x: SW - 3, y: SH - 3, w: 8, h: 8, fill: { color: ACCENT, transparency: 85 } });
    ...
    s.addShape(pptx.ShapeType.rect, { x: PAD_X, y: PAD_Y + 0.1, w: 0.1, h: TITLE_H * 0.8, fill: { color: ACCENT }, rectRadius: 0.5 });
    s.addText(safeString(slide.title), {
    ...
    switch (slide.type) {
      ...
      case 'table':
        if (slide.tableData?.headers && slide.tableData?.rows) {
          ...
          tableRows.push(slide.tableData.headers.map(h => ({
            text: safeString(h),
            options: { fill: PRIMARY, color: WHITE, bold: true, fontSize: CONTENT_PT - 2, fontFace: SAFE_FONT, align: 'center', valign: 'middle', margin: 0.1 }
          })));
          ...
          slide.tableData.rows.forEach((row, rIdx) => {
            ...
            tableRows.push(row.map(cell => ({
              ...
              options: { fill: rowColor, color: DARK, fontSize: CONTENT_PT - 3, fontFace: SAFE_FONT, align: 'center', valign: 'middle', margin: 0.1 }
            })));
          });
      ...
      case 'compare':
        ...
        s.addShape(pptx.ShapeType.path, { x: contentX, y: contentY, w: halfW, h: 0.6, fill: { color: 'F1F5F9' } }); // 헤더 배경
        ...
        if (slide.leftItems) {
            const leftBullets = slide.leftItems.map(item => ({ text: safeString(item), options: { bullet: true, fontSize: CONTENT_PT - 2, color: DARK, fontFace: SAFE_FONT, paraSpaceAfter: 12 } }));
        ...
        s.addShape(pptx.ShapeType.path, { x: rightX, y: contentY, w: halfW, h: 0.6, fill: { color: 'EFF6FF' } }); // 헤더 배경
        ...
        if (slide.rightItems) {
            const rightBullets = slide.rightItems.map(item => ({ text: safeString(item), options: { bullet: true, fontSize: CONTENT_PT - 2, color: DARK, fontFace: SAFE_FONT, paraSpaceAfter: 12 } }));
        ...
        if (Array.isArray(items) && items.length > 0) {
          ...
          if (slide.layout === 'highlight') {
            ...
            const bulletItems = items.map(item => ({
              ...
              options: { bullet: { type: 'bullet' }, fontSize: CONTENT_PT, color: DARK, fontFace: SAFE_FONT, paraSpaceAfter: 16, lineSpacing: 28 }
            }));
