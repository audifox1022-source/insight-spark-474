// ============================================================
// src/lib/export-presentation.tsx (Work AI - PDF/JSON/PPTX export bridge)
// ============================================================
...
function safeHexToRgb(hex: string, rgb: (r: number, g: number, b: number) => any) {
...
function drawWrappedText(page: any, text: string, options: any) {
  page.drawText(String(text || '').slice(0, options.maxChars ?? 1200), options);
}
...
export const exportToPdf = async (presentation: Presentation, ratio: '16:9' | '4:3' = '16:9') => {
  ...
  const toastId = toast.loading(`[${ratio}] 고해상도 PDF 내보내기를 준비하고 있습니다...`);
  ...
  try {
    ...
    for (let i = 0; i < presentation.slides.length; i++) {
      ...
      const textColor = safeHexToRgb(textColorHex, rgb);
      const accentColor = safeHexToRgb(accentColorHex, rgb);
      ...
      page.drawRectangle({
        ...
        width,
        height,
        color: safeHexToRgb(bgColorHex, rgb),
      });
      ...
      page.drawRectangle({ x: 60, y: height - 120, width: 80, height: 4, color: accentColor });
      ...
      const contentList: any[] = Array.isArray(slide.content) ? slide.content : [];
      ...
      if (layout === 'grid' || layout === 'matrix') {
        contentList.slice(0, 4).forEach((item, idx) => {
          ...
          drawWrappedText(page, item.heading || '', {
            x: xPos + 20,
            y: yPos + 160,
            size: 22,
            font: customFont,
          ...
          drawWrappedText(page, item.description || '', {
            x: xPos + 20,
            y: yPos + 120,
            size: 14,
            font: customFont,
        ...
        if (chartRows.length > 0) {
          page.drawText('Data Chart', { x: 60, y: chartTop + 45, size: 12, font: customFont, color: accentColor });
          chartRows.forEach((point, idx) => {
            const yPos = chartTop - idx * 55;
            const barWidth = Math.max(24, (Math.max(0, point.value) / maxValue) * 430);
            page.drawText(point.label.slice(0, 32), { x: 80, y: yPos + 8, size: 14, font: customFont, color: textColor });
            page.drawRectangle({ x: 250, y: yPos, width: barWidth, height: 24, color: accentColor, opacity: 0.85 });
            page.drawText(String(point.value), { x: 700, y: yPos + 7, size: 12, font: customFont, color: textColor });
          });
        } else {
        ...
        contentList.slice(0, 3).forEach((item, idx) => {
          ...
          page.drawRectangle({ x: 820, y: yPos - 12, width: 360, height: 70, color: rgb(0.97, 0.98, 1.0), opacity: 0.9 });
          drawWrappedText(page, item.heading || '', {
            x: 840,
            y: yPos + 32,
            size: 14,
          ...
          drawWrappedText(page, item.description || '', {
            ...
            size: 9,
            font: customFont,
            color: secondarySurfaceTextColor,
            maxWidth: 320,
            maxChars: 130,
          });
        });
        ...
        page.drawText(title, {
          x: Math.max(60, (width - titleWidth) / 2),
          y: height / 2 + 50,
          size: titleSize,
          font: customFont,
          color: textColor,
        ...
        page.drawText(subtitle, {
          x: Math.max(60, (width - subWidth) / 2),
          y: height / 2 - 50,
          size: subSize,
          font: customFont,
          color: accentColor,
    ...
    toast.success(`${ratio} PDF 내보내기가 완료되었습니다.`, { id: toastId });
  } catch (err) {
