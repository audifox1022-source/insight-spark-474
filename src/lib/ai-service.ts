...
// 브라우저 환경이므로 제한을 조금 더 타이트하게 가져갑니다.
...
// ✨ 클라이언트에서 Google Gemini API 직접 호출
async function callGeminiAPI(prompt: string) {
  const API_KEY = import.meta.env.VITE_GEMINI_API_KEY;
  
  if (!API_KEY) {
    throw new Error("VITE_GEMINI_API_KEY가 설정되지 않았습니다. Vercel 환경변수에 추가해주세요.");
  }
...
export const aiService = {
  ...
  async changePersona(body: any) {
    ...
    let stylePrompt = persona === 'jobs' 
      ? `🍎 스티브 잡스 스타일: 극도로 간결하게. 텍스트 최소화. 핵심 메시지 1~2개로 압축. 감성적이고 비전 제시형 카피.`
      : `💼 맥킨지 스타일: MECE 원칙 입각. 명확한 결론형 문장. 3가지 논리적 근거. 팩트/수치 중심의 이성적 어조.`;
