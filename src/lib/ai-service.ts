// ============================================================
// ai-service.ts — 1단계 레이아웃 학습 가이드 강화 버전
// ============================================================
...
// ============================================================
// ✅ 1단계 개선: getSystemPromptCore — 레이아웃 핵심 규칙 추가
// ============================================================
...
// ============================================================
// ✅ 1단계 개선: SLIDE_SCHEMA — 레이아웃 예시 추가
// ============================================================
const SLIDE_SCHEMA = `
...
[content 타입 — 나쁜 예시 ❌]
{
  ...
  "type": "content",
  "content": [
    "올해 우리 회사는 매출이 전년 대비 25% 증가했으며 이는 주요 성과입니다.",
    "신규 고객을 1,200명 확보했고 이는 목표 대비 120% 달성한 수치입니다.",
    "고객 만족도는 87점을 기록했으며 이는 업계 평균보다 높은 수준입니다.",
    "추가로 직원 만족도도 상승했습니다.",
    "내년에는 더 높은 목표를 설정할 계획입니다.",
    "지속적인 성장을 위해 노력하겠습니다."
  ]
}
❌ 문제: 불릿 6개 초과, 80자 초과 서술형 문장, 여러 주제 혼재
...
[chart 타입 — 좋은 예시 ✅]
{
  ...
  "chartData": {
    "type": "bar",
    "labels": ["Q1", "Q2", "Q3", "Q4"],
    "datasets": [{"label": "매출(백만원)", "data": [120, 145, 168, 192]}]
  }
}
✅ 데이터 포인트 4개, 간결한 라벨
...
[chart 타입 — 나쁜 예시 ❌]
{
  ...
  "chartData": {
    "type": "bar",
    "labels": ["2024년 1분기", "2024년 2분기", "2024년 3분기", "2024년 4분기", 
               "2025년 1분기 예상", "2025년 2분기 예상", "2025년 3분기 예상", 
               "2025년 4분기 예상", "2026년 1분기 예상", "2026년 2분기 예상"],
    "datasets": [{"label": "매출", "data": [120, 145, 168, 192, 210, 230, 250, 270, 290, 310]}]
  }
}
❌ 문제: 데이터 포인트 10개 초과 (권장 4~8개), 라벨이 너무 긺
...
[table 타입 — 좋은 예시 ✅]
{
  ...
  "type": "table",
  "tableData": {
    "headers": ["항목", "실적", "목표", "달성률"],
    "rows": [
      ["생산량", "1,200톤", "1,500톤", "80%"],
      ["불량률", "2.1%", "1.5%", "-"],
      ["가동률", "87%", "90%", "97%"]
    ]
  }
...
[table 타입 — 나쁜 예시 ❌]
{
  ...
  "type": "table",
  "tableData": {
    "headers": ["항목", "1월", "2월", "3월", "4월", "5월", "6월", "7월", "8월", "평균"],
    "rows": [
      ["생산량", "100", "110", "120", "130", "140", "150", "160", "170", "135"],
      ["불량률", "2.1", "2.0", "1.9", "1.8", "1.7", "1.6", "1.5", "1.4", "1.75"],
      ["가동률", "85", "86", "87", "88", "89", "90", "91", "92", "88.5"],
      ["인원", "50", "51", "52", "53", "54", "55", "56", "57", "53.5"],
      ["원가", "200", "205", "210", "215", "220", "225", "230", "235", "217.5"]
    ]
  }
}
❌ 문제: 열 9개 초과, 행 5개, 데이터 과다로 가독성 저하
...
[kpi 타입 — 나쁜 예시 ❌]
{
  ...
  "keyMetrics": [
    ...
    {"label": "불량률", "value": "2.1%", "trend": "down"},
    {"label": "가동률", "value": "87%", "trend": "flat"},
    {"label": "인원", "value": "50명", "trend": "flat"},
    {"label": "원가", "value": "200만원", "trend": "down"},
    {"label": "재고", "value": "300톤", "trend": "up"},
    {"label": "매출", "value": "500만원", "trend": "up"}
  ]
}
❌ 문제: 지표 7개 초과 (권장 3~4개), 한 화면에 너무 많은 정보
