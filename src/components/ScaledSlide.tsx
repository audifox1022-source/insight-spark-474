    ...
    <div ref={containerRef} className={`relative overflow-hidden ${containerClassName}`} style={{ aspectRatio: '16/9' }}>
      <div
        ...
        style={{
          ...
          WebkitFontSmoothing: 'antialiased', MozOsxFontSmoothing: 'grayscale', // 폰트 렌더링 최적화
        }}
      >
        <div className={`w-full h-full ${theme.bg} text-slate-900 flex flex-col relative overflow-hidden tracking-tight`}>
          ...
          {watermark && (
            <div className="absolute inset-0 pointer-events-none flex items-center justify-center z-[1] overflow-hidden opacity-[0.04]">
              <span className="text-[240px] font-black tracking-widest uppercase rotate-[-30deg] whitespace-nowrap select-none text-slate-900">{watermark}</span>
            </div>
          )}
          ...
          {isTitle ? (
            <div className={`flex-1 flex flex-col justify-center relative z-[3] ${isHighlight ? 'items-center text-center px-[80px]' : 'items-start px-[180px]'}`}>
              <div className="flex items-center gap-[16px] mb-[40px]">
                ...
                <span className="text-[22px] font-bold tracking-[0.4em] uppercase font-mono" style={{ color: theme.accent }}>{theme.badge}</span>
                <div className="h-[3px] w-[60px]" style={{ background: theme.accent }} />
              </div>
              <h1 className={`text-[100px] font-black leading-[1.2] tracking-[-0.03em] text-slate-900 ${isHighlight ? 'max-w-[1600px]' : 'max-w-[1400px]'} break-keep`}>
                {slide.title}
              </h1>
              ...
              {slide.content && slide.content.length > 0 && (
                <div className={`space-y-[20px] ${isHighlight ? 'max-w-[1200px]' : 'max-w-[1000px]'}`}>
                  {slide.content.map((item, i) => (
                    <p key={i} className="text-[36px] leading-[1.6] font-medium text-slate-600 break-keep">{item}</p>
                  ))}
                </div>
              )}
              {hasMetrics && (
                <div className={`mt-[64px] flex gap-[40px] ${isHighlight ? 'justify-center' : ''}`}>
                  {slide.keyMetrics!.map((m, i) => (
                    <div key={i} className="flex items-center gap-[20px] bg-white px-[36px] py-[24px] rounded-[24px] border border-slate-200 shadow-xl">
                      <span className="text-[64px] font-black text-slate-900">{m.value}</span>
                      <div className="flex flex-col text-left border-l-2 border-slate-100 pl-[24px]">
                        <span className="text-[20px] text-slate-500 font-bold mb-[4px]">{m.label}</span>
            ...
            <div className="flex-1 flex flex-col relative z-[3]">
              <div className="px-[140px] pt-[80px] pb-[20px]">
                <div className="flex items-center gap-[16px] mb-[24px]">
                  <span className="text-[18px] font-bold tracking-[0.3em] uppercase font-mono px-[20px] py-[8px] rounded-full bg-white border border-slate-200 shadow-sm" style={{ color: theme.accent }}>{theme.badge}</span>
                  <div className="w-[3px] h-[24px] bg-slate-300" />
                  <span className="text-[20px] font-mono text-slate-400 font-bold tracking-[0.2em]">{String(slide.slideNumber).padStart(2, '0')}</span>
                </div>
                <h1 className="text-[72px] font-black leading-[1.2] tracking-[-0.03em] max-w-[1300px] text-slate-900 break-keep">{slide.title}</h1>
              </div>
                    ...
                    <div className="space-y-[24px]">
                      <div className="rounded-[24px] p-[48px] relative overflow-hidden shadow-2xl bg-white border-2" style={{ borderColor: theme.accent }}>
                        ...
                        <div className="flex items-start gap-[24px] relative z-10">
                          ...
                          <div><span className="text-[44px] font-bold leading-[1.4] text-slate-900 break-keep">{slide.content[0]}</span></div>
                        </div>
                      </div>
                      {slide.content.slice(1).map((item, i) => (
                        <div key={i} className="flex items-start gap-[20px] px-[32px] py-[24px] rounded-[20px] bg-white border border-slate-200 shadow-sm">
                          ...
                          <span className="text-[32px] font-medium text-slate-700 leading-[1.6] break-keep">{item}</span>
                        </div>
                      ))}
                    </div>
                  )}
                            ...
                            <thead className="sticky top-0 z-10">
                              <tr>
                                {slide.tableData!.headers.map((h, i) => (
                                  <th key={i} className="px-[24px] py-[20px] text-[24px] font-bold text-slate-800 bg-slate-100 border-b-2 border-slate-300 whitespace-nowrap shadow-sm">{h}</th>
                                ))}
                              </tr>
                            </thead>
                            <tbody className="divide-y divide-slate-100 text-slate-700 font-medium">
                              {slide.tableData!.rows.map((row, rIdx) => (
                                <tr key={rIdx} className="hover:bg-slate-50 transition-colors">
                                  {row.map((cell, cIdx) => (
                                    <td key={cIdx} className="px-[24px] py-[24px] text-[22px] leading-[1.5] break-keep">{cell}</td>
                                  ))}
                                </tr>
                              ))}
                            </tbody>
          ...
          /* ======= DATA / CHART / SUMMARY 공용 (레이아웃 마법사) ======= */
            ...
            <div className="flex-1 flex flex-col relative z-[3]">
              <div className={`px-[140px] pt-[80px] pb-[28px] ${isHighlight ? 'text-center' : ''}`}>
                ...
                <h1 className={`text-[72px] font-black leading-[1.2] tracking-[-0.03em] text-slate-900 ${isHighlight ? 'mx-auto max-w-[1600px]' : 'max-w-[1300px]'} break-keep`}>{slide.title}</h1>
              </div>
              ...
              <div className={`flex-1 px-[140px] py-[32px] min-h-0 relative z-[3] ${isGrid ? 'grid grid-cols-2 gap-[72px] items-center' : isHighlight ? 'flex flex-col items-center text-center gap-[48px]' : isSplitLeft ? 'flex flex-row-reverse gap-[72px]' : 'flex flex-row gap-[72px]'}`}>
                ...
                {/* 텍스트 영역 */}
                <div className={`flex flex-col justify-center ${isGrid ? 'w-full' : isHighlight ? 'w-full max-w-[1500px] items-center' : hasVisual ? (hasTable ? 'w-[45%]' : 'flex-1') : 'flex-1'}`}>
                  {slide.content && slide.content.length > 0 && (
                    <ul className={`space-y-[32px] ${isHighlight ? 'flex flex-col items-center' : ''}`}>
                      {slide.content.map((item, i) => (
                        <li key={i} className={`flex ${isHighlight ? 'flex-col items-center text-center gap-[24px]' : 'items-start gap-[24px]'} bg-white p-[32px] rounded-[24px] shadow-sm border border-slate-200`}>
                          {!isHighlight && (
                            <div className="mt-[6px] flex-shrink-0 w-[48px] h-[48px] rounded-[16px] flex items-center justify-center text-[22px] font-black" style={{ background: hexToRgba(theme.accent, 0.1), color: theme.accent }}>
                              {String(i + 1).padStart(2, '0')}
                            </div>
                          )}
                          <span className={`text-[36px] leading-[1.65] font-medium text-slate-800 break-keep ${isHighlight ? 'text-[44px] font-bold' : ''}`}>{item}</span>
                        </li>
                      ))}
                    </ul>
                  )}
                ...
                {/* 시각 자료 영역 */}
                {hasVisual && (
                  <div className={`flex flex-col justify-center gap-[32px] min-h-0 ${isGrid ? 'w-full h-full max-h-[600px]' : isHighlight ? 'w-full max-w-[1500px] flex-row flex-wrap justify-center h-[460px]' : (hasTable ? 'w-[55%] flex-shrink-0' : 'w-[640px] flex-shrink-0')}`}>
                    {hasTable && (
                        ...
                        {slide.keyMetrics!.map((m, i) => {
                          ...
                          return (
                            <div key={i} className={`rounded-[24px] p-[40px] relative overflow-hidden shadow-xl bg-white border border-slate-200 ${isHighlight ? 'w-[420px]' : ''}`}>
                              <div className="flex items-center justify-between mb-[16px]">
                                <span className="text-[24px] text-slate-500 font-bold tracking-wide">{m.label}</span>
                                <span style={{ color: trend.color }}>{trend.icon}</span>
                              </div>
                              <div className="text-[72px] font-black tracking-tight leading-none text-slate-900">{m.value}</div>
                            </div>
                          );
                        })}
                      </div>
