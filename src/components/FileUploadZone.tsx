    ...
    >
      ...
      >
        ...
        <div className="flex flex-col items-center gap-5 relative">
          <motion.div
            className={`${compact ? 'w-12 h-12' : 'w-18 h-18'} rounded-2xl gradient-primary flex items-center justify-center shadow-glow`}
            whileHover={{ scale: 1.05, rotate: 2 }}
            transition={{ type: 'spring', stiffness: 300 }}
          >
            <Upload className={`${compact ? 'w-5 h-5' : 'w-8 h-8'} text-primary-foreground`} />
          </motion.div>
          <div>
            <p className={`${compact ? 'text-base' : 'text-lg'} font-bold text-foreground`}>원본 데이터를 드래그하거나 클릭하여 업로드</p>
            {!compact && (
