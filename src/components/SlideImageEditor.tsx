// ============================================================
// SlideImageEditor.tsx — 배경/이미지 설정 (검색 탭 제거, AI 생성 개선)
// 탭1: 색상/그라디언트 | 탭2: AI 이미지 | 탭3: 프리셋
// ============================================================
import { useState } from 'react';
import { Button } from '@/components/ui/button';
import { Input } from '@/components/ui/input';
import { Loader2, Wand2, X, ImageIcon, Palette, Grid3X3, RefreshCw } from 'lucide-react';
...
const PRESET_IMAGES = {
  ...
  심플: [
    ...
    { id: 's5', thumb: 'https://images.unsplash.com/photo-1579546929518-9e396f3cc809?w=400&h=225&fit=crop', full: 'https://images.unsplash.com/photo-1579546929518-9e396f3cc809?w=1280&h=720&fit=crop', label: '그라데이션' },
    { id: 's6', thumb: 'https://images.unsplash.com/photo-1557683316-973673baf926?w=400&h=225&fit=crop', full: 'https://images.unsplash.com/photo-1557683316-973673baf926?w=1280&h=720&fit=crop', label: '미니멀' },
  ],
};
...
function buildPollinationsUrl(title: string, content: string, seed?: number) {
  ...
  return `https://image.pollinations.ai/prompt/${encodeURIComponent(prompt)}?width=1280&height=720&nologo=true&seed=${s}&nofeed=true`;
}
...
export function SlideImageEditor({
  ...
  const [selectedCategory, setSelectedCategory] = useState<keyof typeof PRESET_IMAGES>('비즈니스');
  ...
  const generateAiImage = () => {
    ...
    setAiImgError(false);
    setIsAiLoading(true);
    setAiImageUrl(''); // 기존 이미지 먼저 지우기 (중복 표시 방지)
    ...
    // 짧은 딜레이 후 URL 설정 (이전 onLoad/onError가 클리어되도록)
    setTimeout(() => setAiImageUrl(url), 50);
  };

  const applyAiImage = () => {
    if (!aiImageUrl || aiImgError) {
      generateAiImage();
      return;
    }
  ...
  const tabs: { key: TabType; icon: React.ReactNode; label: string }[] = [
    ...
    { key: 'ai', icon: <Wand2 className="w-3.5 h-3.5" />, label: 'AI 생성' },
    ...
    <div className="space-y-3">
      ...
      {activeTab === 'color' && (
        <div className="space-y-2">
          <p className="text-[10px] text-muted-foreground font-medium">추천 색상 조합을 클릭해 바로 적용</p>
      ...
      {activeTab === 'ai' && (
        <div className="space-y-3">
          ...
          <div className="flex items-center gap-2">
            ...
            >
              {customPromptMode ? '✓ 직접 입력 중' : '내용 직접 입력'}
            </button>
          </div>
          ...
          <div className="relative w-full aspect-video rounded-xl overflow-hidden border border-border bg-muted/30 flex items-center justify-center">
            {!aiImageUrl && !isAiLoading && (
              <div className="flex flex-col items-center gap-2 text-muted-foreground">
                ...
                <span className="text-[10px]">아래 버튼을 눌러 AI 이미지를 생성하세요</span>
              </div>
            )}
            {isAiLoading && (
            ...
            {aiImgError && (
              <div className="flex flex-col items-center gap-2 text-muted-foreground">
                ...
                <span className="text-[10px]">이미지 로드 실패. 재생성을 눌러보세요</span>
              </div>
            )}
          </div>
          ...
          <div className="flex gap-2">
            ...
            {aiImageUrl && !aiImgError && !isAiLoading && (
              <>
                <Button
                  size="sm"
                  variant="ghost"
      ...
      {activeTab === 'preset' && (
        <div className="space-y-2">
          ...
          <div className="grid grid-cols-2 gap-2">
            {PRESET_IMAGES[selectedCategory].map(img => (
              <button
                key={img.id}
