...
For local smoke checks, start the app with `npm run dev`, open `http://localhost:8080/`, and verify the backend with `http://localhost:3001/api/health`. The same `/api/health` path is available as a Vercel serverless function and returns a safe runtime configuration summary without secret values. `ready: true` means the required Supabase, Gemini, and Blob runtime settings are present and the Supabase project ref matches this repo.
